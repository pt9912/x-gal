...
| **7. Advanced Routing**                                                            | 🔄 In Arbeit     | 2 Wochen   | 🟢 Niedrig |
| **7.1. Envoy (Implementierung, Unit-Tests, E2E-Tests und Dokumentaion)**           | ✅ Abgeschlossen | 2-3 Tage   | 🟢 Niedrig |
...
**Fortschritt:** 6/8 Features abgeschlossen (75%)

---

## Docker Compose E2E Test-Plan (Alle Features)

### Übersicht

**Ziel:** End-to-End-Tests für alle v1.4.0-Features mit realen Provider-Deployments (Docker Compose)

**Test-Strategie:**
- ✅ **Feature 5 (Traffic Splitting)**: Bereits vollständig getestet (6 Provider)
- ✅ **Feature 6 (Request Mirroring)**: Bereits vollständig getestet (6 Provider)
- 🔄 **Feature 1 (gRPC)**: E2E-Tests ausstehend
- 🔄 **Feature 2, 3, 4 (Cloud Provider)**: Manuelle Tests (Cloud-Ressourcen)
- 🔄 **Feature 7 (Advanced Routing)**: E2E-Tests geplant
- 🔄 **Feature 8 (GraphQL)**: E2E-Tests geplant

### Bestehende E2E-Tests (Feature 5 & 6)

**Verzeichnisstruktur:** `tests/docker/`

```
tests/docker/
├── README.md (867 Zeilen - Umfassende Dokumentation)
├── backends/ (Mock-Backends: stable.py, canary.py, mirror.py)
├── envoy/ (Traffic Splitting)
├── envoy-mirroring/ (Request Mirroring)
├── nginx/ (Traffic Splitting)
├── nginx-mirroring/ (Request Mirroring)
├── kong/ (Traffic Splitting)
├── kong-mirroring/ (Request Mirroring)
├── haproxy/ (Traffic Splitting)
├── haproxy-mirroring/ (Request Mirroring)
├── haproxy-mirroring-spoe/ (HAProxy SPOE Mirroring)
├── traefik/ (Traffic Splitting)
├── traefik-mirroring/ (Request Mirroring)
├── apisix/ (Traffic Splitting)
└── apisix-mirroring/ (Request Mirroring)
```

**Bestehende Tests:**
- `tests/test_docker_runtime.py` (Traffic Splitting - 6 Provider)
- `tests/test_*_mirroring_e2e.py` (Request Mirroring - 6 Provider)
  - `test_envoy_mirroring_e2e.py`
  - `test_nginx_mirroring_e2e.py`
  - `test_kong_mirroring_e2e.py`
  - `test_haproxy_mirroring_e2e.py`
  - `test_apisix_mirroring_e2e.py`
  - `test_traefik_mirroring_e2e.py` (Config fertig, Test ausstehend)

### Feature 1: gRPC Transformations E2E-Tests

**Status:** 🔄 Ausstehend

**Verzeichnisstruktur:** `tests/docker/grpc/`

```
tests/docker/grpc/
├── envoy-grpc/
│   ├── docker-compose.yml
│   ├── envoy.yaml (generiert via GAL)
│   └── test_envoy_grpc.py
├── nginx-grpc/
│   ├── docker-compose.yml
│   ├── nginx.conf (generiert via GAL)
│   └── test_nginx_grpc.py
├── backends/
...
**Test-Szenarien:**
1. **gRPC-to-REST Transcoding** (Envoy native, Nginx mit grpc_pass)
2. **Protobuf Message Transformation** (Field mapping)
3. **gRPC Health Checks** (gRPC health check protocol)
4. **gRPC Load Balancing** (Round-robin, Least-request)

**Provider:**
- ✅ Envoy (Native gRPC support)
- ✅ Nginx (grpc_pass directive)
- ⚠️ Kong (gRPC plugin)
- ⚠️ APISIX (grpc-transcode plugin)
- ❌ HAProxy (nur TCP proxy, kein gRPC-spezifisch)
- ❌ Traefik (gRPC routing, kein Transcoding)

**Aufwand:** 1 Woche (4 Provider-Tests)

### Feature 7: Advanced Routing E2E-Tests

**Status:** 🔄 Geplant

**Verzeichnisstruktur:** `tests/docker/advanced-routing/`

```
tests/docker/advanced-routing/
├── envoy-routing/
│   ├── docker-compose.yml
│   ├── envoy.yaml (generiert via GAL)
│   └── test_envoy_routing.py
├── nginx-routing/
│   ├── docker-compose.yml
│   ├── nginx.conf (generiert via GAL)
...
├── traefik-routing/
├── backends/
│   ├── v1_backend.py (Version 1 API)
│   ├── v2_backend.py (Version 2 API)
│   ├── admin_backend.py (Admin API)
│   └── eu_backend.py (EU Data Residency)
└── README.md
```

**Test-Szenarien:**
1. **Header-based Routing** (`X-API-Version: v2` → v2_backend)
2. **JWT Claims Routing** (`role: admin` → admin_backend)
3. **Geo-based Routing** (Geo-IP: DE → eu_backend)
4. **Query Parameter Routing** (`?version=2` → v2_backend)
5. **Combined Rules** (Header + JWT → specific backend)
6. **Fallback Target** (No match → default backend)

**Provider:**
- ✅ Envoy (Route matching + Lua for JWT/Geo)
- ✅ Nginx (map + geo + if + Lua)
- ✅ Kong (Route by Header plugin)
- ✅ APISIX (vars + serverless-pre-function)
- ✅ HAProxy (ACL routing)
- ✅ Traefik (Rule matchers)

**Aufwand:** 1.5 Wochen (6 Provider-Tests)

### Feature 8: GraphQL Support E2E-Tests

**Status:** 🔄 Geplant

**Verzeichnisstruktur:** `tests/docker/graphql/`

```
tests/docker/graphql/
├── kong-graphql/ (Kong Enterprise)
│   ├── docker-compose.yml
│   ├── kong.yml (generiert via GAL)
│   └── test_kong_graphql.py
├── apisix-graphql/
│   ├── docker-compose.yml
│   ├── apisix.yaml (generiert via GAL)
│   └── test_apisix_graphql.py
├── backends/
│   ├── graphql_server.py (Python GraphQL Server - Strawberry/Ariadne)
│   └── schema.graphql
└── README.md
```

**Test-Szenarien:**
1. **Schema Validation** (Valid vs Invalid schema)
2. **Query Depth Limiting** (Max depth: 10)
3. **Query Complexity Limiting** (Max complexity: 1000)
4. **Introspection Control** (Enable/Disable `__schema`, `__type`)
5. **Persisted Queries** (Query ID → pre-stored query)

**Provider (Nur empfohlene):**
- ✅ Kong Enterprise (graphql-rate-limiting-advanced plugin)
- ✅ APISIX (serverless-pre-function with lua-graphql)
- ⚠️ Envoy (WASM filter - requires external binary)
- ⚠️ Nginx (OpenResty + lua-resty-graphql - komplex)
- ❌ HAProxy (nicht empfohlen)
- ❌ Traefik (Go-Plugin erforderlich)

**Aufwand:** 1 Woche (2 Provider-Tests - Kong, APISIX)

### Cloud Provider E2E-Tests (Features 2, 3, 4)

**Status:** ⚠️ Manuelle Tests (Cloud-Ressourcen erforderlich)

**Hinweis:** Docker Compose nicht geeignet für Cloud Provider (AWS, Azure, GCP).
Tests erfordern echte Cloud-Ressourcen und werden manuell oder via Cloud CI/CD durchgeführt.

**Alternative:**
- **LocalStack** (AWS API Gateway Emulator) - Eingeschränkte Funktionalität
- **Azurite** (Azure Storage Emulator) - Kein APIM Support
- **Cloud Provider Terraform Tests** - Automatisierte Infrastruktur-Tests

**Aufwand:** 2 Wochen (3 Cloud Provider - manuelle Tests + Terraform)
...
```python
# tests/docker/backends/grpc_server.py (gRPC Backend)
import grpc
from concurrent import futures
from user_service_pb2 import UserResponse
...
# tests/docker/backends/graphql_server.py (GraphQL Backend)
import strawberry
...
def test_traffic_distribution(url: str, num_requests: int, expected_distribution: Dict[str, float]):
    """Test traffic distribution (for Traffic Splitting tests)."""
    results = {}
    ...
    for backend, expected_pct in expected_distribution.items():
        actual_pct = (results.get(backend, 0) / num_requests) * 100
        assert abs(actual_pct - expected_pct) <= 5, f"{backend}: {actual_pct}% (expected {expected_pct}%)"

def test_request_mirroring(primary_url: str, mirror_url: str, num_requests: int):
    """Test request mirroring."""
    for _ in range(num_requests):
        requests.get(primary_url)

    # Check mirror received requests
    mirror_stats = requests.get(f"{mirror_url}/stats").json()
    assert mirror_stats["request_count"] > 0

def test_grpc_transcoding(grpc_channel: grpc.Channel, user_id: int):
    """Test gRPC-to-REST transcoding."""
    stub = UserServiceStub(grpc_channel)
    response = stub.GetUser(UserRequest(id=user_id))
    assert response.id == user_id
    assert response.name == f"User {user_id}"
```
...
jobs:
  traffic-splitting:
    ...
    steps:
      ...
      - name: Run Traffic Splitting E2E Tests
        run: |
  ...
  request-mirroring:
    ...
    steps:
      - uses: actions/checkout@v3
      - name: Run Request Mirroring E2E Tests
        run: |
          pytest tests/test_*_mirroring_e2e.py -v
  ...
  grpc:
    ...
    steps:
      ...
      - name: Run gRPC E2E Tests
        run: |
          pytest tests/docker/grpc/test_*.py -v

  advanced-routing:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v3
      - name: Run Advanced Routing E2E Tests
        run: |
          pytest tests/docker/advanced-routing/test_*.py -v
  ...
  graphql:
    ...
    steps:
      ...
      - name: Run GraphQL E2E Tests
        run: |
          pytest tests/docker/graphql/test_*.py -v
```

### Zusammenfassung E2E-Test-Plan

| Feature                           | Status          | Provider-Tests                 | Aufwand    | Priorität |
| --------------------------------- | --------------- | ------------------------------ | ---------- | --------- |
| **Feature 1 (gRPC)**              | 🔄 Ausstehend    | 4 (Envoy, Nginx, Kong, APISIX) | 1 Woche    | 🟡 Mittel  |
| **Feature 5 (Traffic Splitting)** | ✅ Abgeschlossen | 6 (Alle Self-Hosted)           | -          | -         |
| **Feature 6 (Request Mirroring)** | ✅ Abgeschlossen | 6 (Alle Self-Hosted)           | -          | -         |
| **Feature 7 (Advanced Routing)**  | 🔄 Geplant       | 6 (Alle Self-Hosted)           | 1.5 Wochen | 🟢 Niedrig |
| **Feature 8 (GraphQL)**           | 🔄 Geplant       | 2 (Kong, APISIX)               | 1 Woche    | 🟢 Niedrig |
| **Cloud Provider (Features 2-4)** | ⚠️ Manuell       | 3 (AWS, Azure, GCP)            | 2 Wochen   | 🟢 Niedrig |

**Gesamt-Aufwand:** ~5.5 Wochen für alle ausstehenden E2E-Tests

**Empfohlene Reihenfolge:**
1. ✅ Feature 5 & 6 (bereits abgeschlossen)
2. Feature 7 (Advanced Routing) - Wichtigste ausstehende Tests
3. Feature 1 (gRPC) - Wichtig für gRPC-Support-Validierung
4. Feature 8 (GraphQL) - Nur 2 Provider (Kong, APISIX)
5. Cloud Provider (Manuell/Terraform) - Niedrigste Priorität
