...
- **Lösung**: GAL bietet eine einheitliche Konfiguration für GraphQL-APIs mit Schema Management und Query Analysis

### Warum GraphQL Support?

**Use Cases:**
- GraphQL API Gateway für Microservices
- Schema Validation & Introspection Control
- Query Complexity & Depth Limiting (DoS Protection)
- GraphQL-to-REST Translation (Legacy Backend Integration)
- Query Caching & Persisted Queries
- Field-Level Authorization

**Vorteile:**
- ✅ **Schema Validation**: GraphQL Schema (.graphql) Management
- ✅ **Query Analysis**: Complexity & Depth Limits
- ✅ **Security**: Introspection Control, Query Whitelisting
- ✅ **Performance**: Query Caching, Persisted Queries
...
def _generate_graphql_config_envoy(self, route):
    ...
    logger.warning(
        "GraphQL support in Envoy requires external service or Lua filter.\n"
        "Options:\n"
        "  1. ext_authz filter with external GraphQL validator\n"
        "  2. Lua filter with graphql-lua library\n"
        "  3. Deploy Apollo Router/GraphQL Mesh as sidecar"
    )

    # Return ext_authz config
    return {
        ...
        "typed_config": {
            ...
            "grpc_service": {
                "envoy_grpc": {
                    "cluster_name": "graphql_validator_service"
                }
            }
        }
    }
...
def _generate_graphql_config_haproxy(self, route):
    ...
    logger.warning(
        "GraphQL support in HAProxy requires external Lua scripts.\n"
        "Steps:\n"
        "  1. Install lua-graphql: luarocks install lua-graphql\n"
        "  2. Create Lua script: /etc/haproxy/lua/graphql_validator.lua\n"
        "  3. Load in global: lua-load /etc/haproxy/lua/graphql_validator.lua\n"
        "  4. Reference in backend: http-request lua.graphql_validate"
    )

    return [
        "    # GraphQL Validation (requires Lua script)",
...
def _generate_graphql_config_traefik(self, route):
    """Generate Traefik GraphQL Middleware."""
    if not route.graphql or not route.graphql.enabled:
        return None

    logger.warning(
        "GraphQL support in Traefik requires custom plugin or ForwardAuth.\n"
        "Options:\n"
        "  1. ForwardAuth Middleware with external GraphQL validator\n"
        "  2. Custom Traefik Plugin (Go development)\n"
        "  3. Deploy Apollo Router/GraphQL Mesh separately"
    )

    return {
        "forwardAuth": {
            "address": "http://graphql-validator:8080/validate",
...
- **graphql-core** (Python GraphQL implementation)
- **graphql-lua** (Nginx/APISIX/HAProxy Lua validation)
- **Apollo Router** (Optional: Envoy/Traefik integration)
- **GraphQL Mesh** (Optional: REST-to-GraphQL translation)

### Provider Support Matrix

| Provider | Native Support | Implementation | Notes |
|----------|---------------|----------------|-------|
| Envoy | ❌ | ext_authz | External validator service |
| Nginx | ⚠️ | Lua | graphql-lua library |
| Kong | ✅ | Enterprise | graphql-rate-limiting-advanced |
| APISIX | ⚠️ | Lua | serverless-pre-function |
| HAProxy | ❌ | Lua | External lua-graphql script |
| Traefik | ❌ | ForwardAuth | External validator |
| AWS | ✅ | AppSync | Native GraphQL service |
| Azure APIM | ✅ | Native | GraphQL Passthrough API |
| GCP | ✅ | Endpoints | Cloud Endpoints + ESPv2 |
