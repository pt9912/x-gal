...
class TestNginxRequestMirroringE2E:
    ...
    def docker_compose_file(self):
        ...
        return str(Path(__file__).parent / "docker" / "nginx-mirroring" / "docker-compose.yml")
        ...
        for i in range(num_requests):
            try:
                ...
                response = requests.post(
                    "http://localhost:10002/api/v1", json=payload, timeout=5
                )
    ...
    def test_nginx_configuration_verification(self, nginx_mirroring_setup):
        ...
        compose_dir = Path(__file__).parent / "docker" / "nginx-mirroring"
